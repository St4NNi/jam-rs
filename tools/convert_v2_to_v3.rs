...
fn convert(input_path: &PathBuf, output_path: &PathBuf) -> Result<(), ConvertError> {
    ...
    for (_i, meta) in bucket_metas.iter().enumerate() {
    ...
    output_data[56..64].copy_from_slice(&(HEADER_SIZE as u64).to_le_bytes());
    let bucket_start = bucket_regions_start as u64;
    output_data[64..72].copy_from_slice(&bucket_start.to_le_bytes());
    output_data[72..80].copy_from_slice(&bucket_start.to_le_bytes());
