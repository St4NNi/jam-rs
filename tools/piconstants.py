...
fn calculate_pi_minus_3(precision_digits: usize) -> BigRational {
    ...
    fn arctan_series(x_inv: i64, terms: usize) -> BigRational {
        ...
        for n in 1..terms {
            ...
            let next_term = &term * BigRational::from(BigInt::from(sign)) / BigRational::from(BigInt::from(denominator));
    ...
    let pi_over_4 = BigRational::from(BigInt::from(4)) * arctan_series(5, terms) - arctan_series(239, terms);
...
fn generate_constants(range_size: usize) -> Vec<u64> {
    println!("Generating {} constants with pure Rust precision...", range_size - 1);
    
    // Calculate π - 3 with high precision
    let start_calc = Instant::now();
    let a = calculate_pi_minus_3(1000); // 1000 decimal digits should be plenty
    println!("Calculated π - 3 in {:.2}s", start_calc.elapsed().as_secs_f64());
...
fn find_best_triplet_exhaustive(constants: &[u64]) -> (u32, (usize, usize, usize), (u64, u64, u64)) {
    let n = constants.len();
        ...
        for j in (i + 1)..n {
            for k in (j + 1)..n {
                ...
                if processed % 100_000 == 0 {
                    ...
                    println!("Processed {:>10} triplets ({:>6.2}%) | Best: {} | Rate: {:>8.0}/sec | ETA: {:>6.1}s", 
                             processed, 
...
fn find_best_triplet_sampled(constants: &[u64], sample_size: usize) -> (u32, (usize, usize, usize), (u64, u64, u64)) {
    use rand::prelude::*;
    ...
    for i in 0..sample_size {
        ...
        if idx3 >= idx1.min(idx2) { idx3 += 1; }
        if idx3 >= idx1.max(idx2) { idx3 += 1; }
        ...
        if i % 10_000 == 0 && i > 0 {
            ...
            println!("Processed {:>8} samples | Best: {} | Rate: {:>8.0}/sec | ETA: {:>6.1}s", 
...
fn main() {
    ...
    println!("Generated {} constants in {:.2}s\n", constants.len(), start_time.elapsed().as_secs_f64());
    ...
    println!("Achieved distance: {} bits ({:.1}% of maximum)", 
             max_distance, max_distance as f64 / 192.0 * 100.0);
