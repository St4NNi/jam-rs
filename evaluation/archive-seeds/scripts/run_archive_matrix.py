#!/usr/bin/env python3
"""Plan or run native-JMA and Pithos archive policy measurements."""
...
def main() -> int:
    ...
    try:
        result = run_manifest(load_manifest(args.manifest, "archive_matrix"), execute=args.execute)
        write_new(args.output, result, pretty=args.pretty)
    except (ExperimentError, OSError) as exc:
        parser.exit(1, f"archive matrix failed: {exc}\n")
