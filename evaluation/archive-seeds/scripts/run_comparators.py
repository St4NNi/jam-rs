#!/usr/bin/env python3
"""Plan or run matched-recall BLASTn and minimap2 comparator measurements."""
...
def main() -> int:
    ...
    try:
        result = run_manifest(load_manifest(args.manifest, "comparators"), execute=args.execute)
        write_new(args.output, result, pretty=args.pretty)
    except (ExperimentError, OSError) as exc:
        parser.exit(1, f"comparator run failed: {exc}\n")
