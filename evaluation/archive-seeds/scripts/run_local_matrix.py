#!/usr/bin/env python3
"""Plan or run local mmap query measurements."""
...
def main() -> int:
    ...
    try:
        result = run_manifest(load_manifest(args.manifest, "local_matrix"), execute=args.execute)
        write_new(args.output, result, pretty=args.pretty)
    except (ExperimentError, OSError) as exc:
        parser.exit(1, f"local matrix failed: {exc}\n")
