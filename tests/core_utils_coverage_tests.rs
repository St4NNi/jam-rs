...
use jam_rs::core_utils::{
    FileMetadata, HashMetadata, TopNHashCollector, calculate_gc_content, categorize_gc_percent,
    categorize_length, passes_entropy_filter, shannon_entropy,
};
...
fn test_hash_metadata_pack_unpack() {
    ...
    assert!(
        metadata.length_category > 0,
        "Length category should be calculated"
    );
}
...
fn test_calculate_gc_content() {
    ...
    assert!(
        gc_content > 0.0 && gc_content < 100.0,
        "Mixed sequence should have intermediate GC content"
    );
    ...
    assert_eq!(
        gc_with_n, gc_without_n,
        "N's should be ignored in GC calculation"
    );
}
...
fn test_gc_content_edge_cases() {
    ...
    assert_eq!(
        calculate_gc_content(lowercase),
        calculate_gc_content(uppercase)
    );
    ...
    let clean = b"ATCG";
    assert_eq!(
        calculate_gc_content(with_unknown),
        calculate_gc_content(clean)
    );
}
